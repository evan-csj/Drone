...
float Kp[3] = {0, 10, 10};    // P coefficients in that order : Yaw, Pitch, Roll
float Ki[3] = {0, 0.01, 0.01}; // I coefficients in that order : Yaw, Pitch, Roll
float Kd[3] = {0, 1000, 1000};        // D coefficients in that order : Yaw, Pitch, Roll
...
void pidController() {
    ...
    if (throttle >= 1000) {
        // PID = e.Kp + ∫e.Ki + Δe.Kd
        yaw_pid   = (errors[YAW]   * Kp[YAW])   + (iErrors[YAW]   * Ki[YAW])   + (dErrors[YAW]   * Kd[YAW]);
        pitch_pid = (errors[PITCH] * Kp[PITCH]) + (iErrors[PITCH] * Ki[PITCH]) + (dErrors[PITCH] * Kd[PITCH]);
        roll_pid  = (errors[ROLL]  * Kp[ROLL])  + (iErrors[ROLL]  * Ki[ROLL])  + (dErrors[ROLL]  * Kd[ROLL]);
        ...
        pwm1 = throttle + roll_pid + pitch_pid - yaw_pid;
        pwm2 = throttle - roll_pid + pitch_pid + yaw_pid;
        pwm3 = throttle - roll_pid - pitch_pid - yaw_pid;
        pwm4 = throttle + roll_pid - pitch_pid + yaw_pid;
    }

    // Prevent out-of-range-values
    pwm1 = minMax(pwm1, 1000, 2000);
    pwm2 = minMax(pwm2, 1000, 2000);
    pwm3 = minMax(pwm3, 1000, 2000);
    pwm4 = minMax(pwm4, 1000, 2000);
}
